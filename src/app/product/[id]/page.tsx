...
import { ProductShowcase } from "../../../remotion/ProductShowcase";
import {
  MOCK_PRODUCTS,
  getProductColors,
  getColorHex,
...
// Dynamically import Remotion Player to prevent SSR errors
const Player = dynamic(() => import("@remotion/player").then((mod) => mod.Player), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full bg-zinc-950 flex items-center justify-center text-zinc-500 font-outfit min-h-[300px]">
      <div className="flex flex-col items-center gap-3">
        <div className="w-8 h-8 border-4 border-purple-500/20 border-t-purple-500 rounded-full animate-spin" />
        <span className="text-[10px] font-bold tracking-widest uppercase text-zinc-500">Loading Showcase...</span>
      </div>
    </div>
  ),
});
...
export default function ProductDetailPage() {
  ...
  // Find product by id with state and database fetch
      ...
      try {
        ...
        if (data) {
          const mapped: Product = {
            ...
            price: typeof data.price === "number" ? `$${Math.round(data.price)}` : data.price,
            slashedPrice: data.slashed_price ? `$${Math.round(data.slashed_price)}` : "",
        ...
        console.warn("Failed to load product from Supabase, using local fallback:", e);
      }
    }
    loadProduct();
  }, [id]);
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center gap-4 text-center font-outfit px-6">
        <h2 className="text-2xl font-extrabold text-zinc-900 font-syne">Product Not Found</h2>
        <p className="text-zinc-500 text-sm max-w-xs">We couldn't find the product you were looking for. It may have been removed or discontinued.</p>
        <Link href="/" className="bg-zinc-950 text-white text-xs font-bold tracking-widest px-6 py-3.5 rounded-full hover:bg-zinc-800 transition-all shadow-md">
          BACK TO STOREFRONT
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 flex flex-col font-outfit select-none relative">
      ...
      <div 
        className="absolute top-0 right-1/4 w-[450px] h-[450px] blur-[150px] rounded-full pointer-events-none opacity-10 transition-all duration-[600ms] ease-out z-0"
        style={{ background: `radial-gradient(circle, ${activeColorHex} 0%, transparent 70%)` }}
      />
      ...
      <main className="flex-grow flex items-center justify-center py-8 lg:py-16 px-6 relative z-10">
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          ...
          {/* Left Column: Remotion Showcase Player (col-span-6) */}
          <div className="lg:col-span-6 flex flex-col items-center">
            ...
            <div className="w-full aspect-square bg-zinc-950 border border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-zinc-950/20 relative">
              <Player
                key={`detail-${product.id}-${activeColor}`}
                component={ProductShowcase as React.ComponentType<any>}
                inputProps={{
                  productId: product.id,
                  variantColorHex: activeColorHex,
                  features: product.features || PRODUCT_FEATURES[product.id] || [],
            ...
            <span className="text-[9px] text-zinc-400 font-bold tracking-widest uppercase mt-4 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              Real-Time Dynamic Interactive Render
            </span>
          </div>
            ...
            <div className="border-t border-zinc-200/60 pt-5 flex flex-col gap-2.5">
              ...
              <div className="flex items-center gap-1.5 bg-zinc-100 border border-zinc-200/60 rounded-full px-2.5 py-1.5 shadow-inner w-fit">
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-zinc-800 rounded-full hover:bg-zinc-200/60 transition-colors border-0 bg-transparent font-bold cursor-pointer"
                >
                  —
                </button>
              ...
              <div className="p-6 border-t border-zinc-100 bg-zinc-50 flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-xs font-bold text-zinc-400 uppercase tracking-widest">
                    <span>Subtotal</span>
                    <span className="text-zinc-950 font-extrabold">${(cart.reduce((sum, item) => sum + parsePrice(item.product.price) * item.quantity, 0)).toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-2 text-sm font-extrabold text-zinc-950 font-outfit uppercase tracking-widest">
                    ...
                    <span className="text-purple-950 text-base font-black font-outfit">${(cart.reduce((sum, item) => sum + parsePrice(item.product.price) * item.quantity, 0)).toFixed(2)}</span>
                  </div>
                </div>
